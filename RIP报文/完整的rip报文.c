...
void genUDPPacket(pcap_t *_adhandle,int packetNumber)
{
    ...
    memset( &ripHeader, 0, sizeof ripHeader );
    memset( &ripData, 0, sizeof ripData );
    ...
    ipHeader.tos = 0;
    ipHeader.total_len = htons( sizeof ipHeader + sizeof udpHeader + sizeof ripHeader + sizeof ripData );//16位总长度（字节）
    ...
    udpHeader.len  = htons( sizeof udpHeader + sizeof ripHeader + sizeof ripData );
    udpHeader.checkl = 0;
    ...
    /*rip版本2的构造方法
    //填充rip头部
    ripHeader.command = 1;  //1是request ， 2是response
    ripHeader.version = 2;
    ripHeader.pad0    = 0;
    //填充rip数据
    ripData.addrfamily  = 0;
    ripData.routetag   =  ;  //外部路由标记
    ripData.ipaddr    =  inet_addr( ) ;   //该路由的目的IP地址，可以是自然网段的地址，也可以是子网地址或主机地址。
    ripData.netmask   =  inet_addr( );  //目的地址的掩码。
    ripData.nexthop   = inet_addr( ) ;  //提供一个更好的下一跳地址。
    ripData.metric    = 16 ;  //路由的开销值。对于Request报文，此字段为16
    */
   //版本1的构造方法
   //填充rip头部
    ripHeader.command = 1;  //1是request ， 2是response
    ...
    ripHeader.pad0    = 0;
    //填充rip数据
    ripData.addrfamily  = htons(2);//2
    ripData.routetag   =  0;  //外部路由标记
    ripData.ipaddr    =  inet_addr( szSrcIp )  ; //随便写一个地址  //该路由的目的IP地址，可以是自然网段的地址，也可以是子网地址或主机地址。
    ripData.netmask  =  0  ;  //目的地址的掩码。
    ripData.nexthop  = 0  ;  //提供一个更好的下一跳地址。
    ripData.metric   =  16  ;  //路由的开销值。对于Request报文，此字段为16
    ...
    memcpy( ucSend + sizeof psdHeader + sizeof udpHeader, &ripHeader, sizeof ripHeader );
    memcpy( ucSend + sizeof psdHeader + sizeof udpHeader + sizeof ripHeader, &ripData, sizeof ripData );
    udpHeader.checkl = CheckSum((USHORT *)ucSend, sizeof psdHeader + sizeof udpHeader + sizeof ripHeader + sizeof ripData);
    udpHeader.checkl = htons( ntohs( udpHeader.checkl ) - (USHORT)(sizeof ripHeader + sizeof ripData));
    ...
    memcpy( ucSend + sizeof etherHeader + sizeof ipHeader + sizeof udpHeader, &ripHeader, sizeof ripHeader  );
    memcpy( ucSend + sizeof etherHeader + sizeof ipHeader + sizeof udpHeader + sizeof ripHeader, &ripData, sizeof ripData );
    datasize = sizeof ipHeader + sizeof udpHeader + sizeof etherHeader + sizeof ripHeader + sizeof ripData;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
