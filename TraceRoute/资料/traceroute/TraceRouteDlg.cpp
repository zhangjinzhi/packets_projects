...
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CTraceRouteDlg)
	DDX_Text(pDX, IDC_STATIC_SHOW, m_strResult);
	DDX_Text(pDX, IDC_EDIT_ADDRESS, m_strAddress);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CTraceRouteDlg, CDialog)
	//{{AFX_MSG_MAP(CTraceRouteDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	...
	//}}AFX_MSG_MAP
	ON_EN_CHANGE(IDC_EDIT_ADDRESS, &CTraceRouteDlg::OnEnChangeEditAddress)
END_MESSAGE_MAP()
...
//添加并且显示结果信息
void CTraceRouteDlg::InfoAdd(CString info)
{
    m_strResult+=info+"\n\r";
	UpdateData(FALSE);
}

void CTraceRouteDlg::OnButtonTrace() 
{
	...
	UpdateData(TRUE);
	m_strResult="";
	InfoAdd("开始跟踪。   目标："+m_strAddress);
	UpdateData(FALSE);
	pTrc->Trace (m_strAddress.GetBuffer(0));
	InfoAdd("跟踪完成。");
	UpdateData(FALSE);

}

void CTraceRouteDlg::OnButton2() 
{
...
void CTraceRouteDlg::OnEnChangeEditAddress()
{
